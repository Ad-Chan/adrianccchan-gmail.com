...
main(int argc, char * argv[])
{
        ...
        printf("**********\n* opening new file \"test.file\" as write only\n");
        fd = open("test.file", O_WRONLY | O_CREAT, 0700); /* mode u=rw in octal */
        ...
        if (r < 0) {
                printf("ERROR reading file: %s (EXPECTED)\n", strerror(errno));
        } else {
        ...
        printf("**********\n* opening old file \"test.file\" read only\n");
        ...
        printf("* wrote %d bytes\n", r);
        if (r < 0) {
                printf("ERROR writing file: %s (EXPECTED)\n", strerror(errno));
        } else {
                exit(1);
        }
        printf("* closing file\n");
